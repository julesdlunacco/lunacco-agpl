...
export function calculateAspects(planets: AstroPlanetLike[]): ComputedAspect[] {
  const pts = planets.filter(p => p.name !== 'Black Moon Lilith');
...
function detectChartShape(planets: AstroPlanetLike[]): string {
  ...
  const isBucket = (): boolean => {
    ...
    for (let i = 0; i < lons.length; i++) {
      ...
      // pair should be adjacent internally but isolated from the bowl cluster
  ...
  // Two opposing clusters with open space between sides
  if (groupsWide.length === 2 && groupsWide[0].length >= 2 && groupsWide[1].length >= 2) {
    return 'Seesaw';
  }

  // Evenly distributed around full wheel
  if (occupiedArc > 240 && std <= 10 && minGap >= 12) {
    return 'Splash';
  }

  // Uneven spread with multiple clumps
...
function detectPatterns(planets: AstroPlanetLike[], aspects: ComputedAspect[]): string[] {
  const alerts: string[] = [];

  const tight = aspects.filter(a => a.orb <= 2);
  if (tight.length) {
    alerts.push(`Tight aspects: ${tight.slice(0, 3).map(a => `${a.p1.name} ${a.aspect.symbol} ${a.p2.name} (${a.orb.toFixed(1)}°)`).join(', ')}`);
  }
  ...
  const mysticTriangles = triples.filter(([a, b, c]) => {
    const names = [
      aspectNameBetween(a, b, aspects),
      aspectNameBetween(a, c, aspects),
      aspectNameBetween(b, c, aspects),
    ];
    return names.filter(n => n === 'Trine').length === 1 && names.filter(n => n === 'Sextile').length === 2;
  });
  if (mysticTriangles.length) alerts.push(`Mystic Triangle detected (${mysticTriangles[0].map(p => p.name).join(' / ')}).`);
  ...
  const kite = quads.find(group => {
    const tris = combinations(group, 3).find(t =>
      aspectNameBetween(t[0], t[1], aspects) === 'Trine' &&
      aspectNameBetween(t[0], t[2], aspects) === 'Trine' &&
      aspectNameBetween(t[1], t[2], aspects) === 'Trine'
    );
    if (!tris) return false;
    const fourth = group.find(p => !tris.includes(p));
    if (!fourth) return false;
    const oppCount = tris.filter(t => aspectNameBetween(t, fourth, aspects) === 'Opposition').length;
    const sextCount = tris.filter(t => aspectNameBetween(t, fourth, aspects) === 'Sextile').length;
    return oppCount === 1 && sextCount === 2;
  });
  if (kite) alerts.push(`Kite pattern detected (${kite.map(p => p.name).join(' / ')}).`);
...
export function formatMoonPhaseSummary(phaseName: string, moonSign: string, moonDegree: string, dt?: DateTime): string {
  const when = dt ? dt.toFormat('MMM d, yyyy HH:mm') : '';
  const prefix = when ? `${when} · ` : '';
  return `${prefix}${phaseName} in ${moonSign} (${moonDegree})`;
}
